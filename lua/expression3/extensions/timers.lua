...
local extention = EXPR_LIB.RegisterExtenstion("time");

extention:RegisterLibrary("timer");

extention:RegisterFunction("timer", "simple", "n,f,...", "", 0, function(ctx, d, f, ...)
...
extention:RegisterFunction("timer", "create", "s,n,n,f,...", "", 0, function(ctx, n, d, r, f, ...)
	local timers = ctx.data.timers;
...
extention:RegisterFunction("timer", "remove", "s", "", 0, function(ctx, name)
	local timers = ctx.data.timers;
	timers[name] = nil;
end, false);

extention:RegisterFunction("timer", "pause", "s", "", 0, function(ctx, name)
	local timers = ctx.data.timers;
...
extention:RegisterFunction("timer", "resume", "s", "", 0, function(ctx, name)
	local timers = ctx.data.timers;
